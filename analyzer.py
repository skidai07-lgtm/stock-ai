...
[동일업종 경쟁사 비교 (PER, PBR, ROE)]
{stock_data.get('동일업종비교', '정보 없음')}
...
2. **경쟁사 비교 분석**: 제공된 동일업종 경쟁사들의 PER, PBR, ROE와 이 회사를 직접 비교하여 경쟁력과 고평가/저평가 여부를 분석하세요.
3. **모멘텀 및 이슈 분석 (뉴스 기반)**: 주어진 최근 뉴스 헤드라인을 바탕으로 이 회사에 수주, 실적 발표, 악재 등 어떤 모멘텀이 있는지 분석하세요.
4. **업황 전망 및 해외 매출 성장성**: 이 산업군이 레드오션인지 블루오션인지, 그리고 글로벌 경쟁력이 있는지 평가하세요.
5. **종합 판단 및 최종 등급 (A ~ F)**: 위 모든 것을 종합하여 최종 투자 매력도 등급(A, B, C, D, E, F)을 부여하고 핵심 이유를 한 줄로 요약하세요.
